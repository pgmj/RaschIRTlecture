...
psychometric_criteria <- data.frame(
  ...
  Criterion = c(
    "Unidimensionality & local independence", "Ordered response categories",
    "Invariance",
  ...
  Description = c(
    ...
    "Item and measure properties are consistent between relevant demographic groups (gender, age, ethnicity, time, etc). Test-retest correlation is not an invariance test unless it is used to assess item properties (not person scores).",
...
-   Scientist at [RISE Research Institutes of Sweden](https://www.ri.se/en/shic)
...
RIscoreSE(pss7, output = "dataframe") %>% 
  janitor::clean_names() %>% 
  slice(2:28) %>% 
  ggplot(aes(x = logit_score, y = logit_std_error)) +
    ...
    geom_line(group = 1) +
  theme_rise() +
  labs(title = "Distribution of measurement uncertainty",
       x = "Latent score (theta) in logits",
       y = "Standard error of measurement (logits)")
```
...
pca <- pca(std.resids, nfactors = 5, rotate = "oblimin")
...
```{r}
RIloadLoc(pss14) + coord_cartesian(ylim = c(-0.8, 1.25), xlim = c(-0.5, 0.5))
```
...
::: columns
::: {.column width="30%"}
It is the relative correlation between items that is important, not the absolute value [@christensen2017]. Similarly to item fit, we need bootstrapping/simulation to determine cutoff values to interpret the results.
:::

::: {.column width="70%"}
```{r}
#| fig-width: 5
simcor <- RIgetResidCor(pss14, 200, 8)
RIresidcorr(pss14, cutoff = simcor$p99)
```
:::
:::
...
```{r}
itemlabels_fs %>% 
  mutate(itemnr = gsub("flourish","",itemnr)) %>% 
  kbl_rise()
```
:::
...
::: {.column width="60%"}
```{r}
simcor2 <- RIgetResidCor(fs_data, 200, 8)
RIresidcorr(fs_data, cutoff = simcor2$p99)
```
:::
:::
...
```{r}
simfit <- RIgetfit(pss7p, 250, 8)
RIgetfitPlot(simfit, pss7p)
```
...
#| layout-ncol: 2
pss7p %>% 
  mutate(q4p = car::recode(q4p,"1=0;2=1;3=2;4=3")) %>%
...
```{r}
#| fig-width: 7
ordinal_interval <- RIscoreSE(pss7, output = "dataframe") %>% 
  janitor::clean_names()
...
The Test Information Function [@samejima1969; @samejima1994] is basically the inverse of the standard error of measurement.
...
-   CTT: default estimator ML used (or estimator not reported), when WLSMV is appropriate (which it usually is for ordinal data) and using ML-based cutoff values (i.e Hu & Bentler, 1999) that are inappropriate for WLSMV
...
There are guides and tools available for doing Bayesian IRT in R. The `brms` package is highly recommended [@bürkner2017; @bürkner2021] and this excellent blog post is very helpful to get started: <https://solomonkurz.netlify.app/blog/2021-12-29-notes-on-the-bayesian-cumulative-probit/>
