...
psychometric_criteria <- data.frame(
  ...
  Description = c(
    "Items represent one latent variable, without strongly correlated item residuals. Principal Component Analysis and Exploratory Factor Analysis of raw data are explorative methods.",
...
-   Examples using the Rasch/1PL model
    -   if we have time, we'll walk through a full analysis from a recent paper

### Useful links

-   Github repo for this presentation: <https://github.com/pgmj/>
-   The slides can be followed at: \<\>
    -   and downloaded here:
-   My email: [magnus.p.johansson\@ri.se](mailto:magnus.p.johansson@ri.se){.email}

## Latent constructs {background-color="#009ca6"}
...
## Classical vs modern test theory {.smaller background-color="#009ca6"}
...
-   Only Rasch (IRT 1PL) models can justify ordinal sum scores as a sufficient metric to represent measurement on the latent construct scale
    -   Also... Rasch is a special case, see [@andrich2004].
...
-   IRT: not reporting enough aspects/metrics (i.e only reporting item fit)
...
We can check this by looking at the item characteristic curves (ICC). So far, we have only seen ICCs with ordered response categories. Let's look at an example with disordered response categories.
...
```{r}
#| echo: false
# Load data and item information
fs_data <- read_excel("data/data_FS_Didino2019.xlsx") %>% 
  select(starts_with("flourish", ignore.case = FALSE)) 
itemlabels <- read_excel("data/itemlabels_SWLS_FS.xlsx", sheet = 3)
